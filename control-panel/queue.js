 ...
 */
class Queue extends TerminalItemBox {
  constructor(params) {
    super(params);
  ...
  _changeOrderInSongs(boxChildrenIndex1, boxChildrenIndex2) {
    const songsArrayIndex1 =
      this._boxChildrenIndexToSongsIndex(boxChildrenIndex1);
    const songaArrayIndex2 =
      this._boxChildrenIndexToSongsIndex(boxChildrenIndex2);
    [this._songs[songsArrayIndex1], this._songs[songaArrayIndex2]] = [
      this._songs[songaArrayIndex2],
      this._songs[songsArrayIndex1],
    ];
  }
  ...
  _changeOrderInBoxChildren(key) {
    ...
    [child1.content, child2.content] = [
      `${Utils.getFirstWord(child1.content)} ${Utils.discardFirstWord(
        child2.content
      )}`,
      `${Utils.getFirstWord(child2.content)} ${Utils.discardFirstWord(
        child1.content
      )}`,
    ];
